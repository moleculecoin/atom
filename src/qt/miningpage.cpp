...
void MiningPage::minerError(QProcess::ProcessError error)
{
    ...
    {
        reportToList("There is problem with collider!  Run!", ERROR, NULL);
    }
}
...
{
    if (getMiningType() == ClientModel::SoloMining)
        reportToList("Solo Lab stop", ERROR, NULL);
    else
        reportToList("Research Park Particle Collider exit", ERROR, NULL);
    ui->list->addItem("");
    minerActive = false;
    resetMiningButton();
...
void MiningPage::minerStarted()
{
    if (!minerActive)
        if (getMiningType() == ClientModel::SoloMining)
            reportToList("The Solo Lab collider has started!  Hide!", ERROR, NULL);
        else
            reportToList("Research Park collider has started.  Make big boom.", STARTED, NULL);
    minerActive = true;
    resetMiningButton();
...
{
    ui->startButton->setText(minerActive ? "Stop Particle Collider" : "Start Particle Collider");
