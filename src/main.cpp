...
// Copyright (c) 2013 The Atom developers
...
static const unsigned int timeGenesisBlock = 1387098870;
...
const string strMessageMagic = "Atom Signed Message:\n";
...
static const int64 nBlockRewardStartCoin = 1024 * COIN;//2048 * COIN;
...
int64 static GetBlockValue(int nHeight, int64 nFees, unsigned int nBits)
{
    ...
    // Subsidy is cut in half every 48840 blocks (42 days)
    nSubsidy >>= (nHeight / 48840);
...
unsigned int ComputeMinWork(unsigned int nBase, int64 nTime)
{
    ...
    {
        // Maximum 30% adjustment...
        bnResult *= 99/76;
        // ... per timespan
        nTime -= nTargetTimespan*4;  //4x target time
    }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    int nIntervalSmoothing = nInterval * 4;//nInterval-1;
	...
	// ---begin avg
	// Average over 4x interval - concept from FeatherCoin
	//int nIntervalSmooth * 4;

        //const CBlockIndex* pindexFirst = pindexLast;
        //for(int i = 0; pindexFirst && i < nIntervalSmooth; i++)
          //pindexFirst = pindexFirst->pprev;
        ...
        printf("RETARGET: nActualTimespanLong = %d, nActualTimeSpanAvg = %d, nActualTimespan (damped) = %"PRI64d"\n",
          nActualTimespanLong, nActualTimespanAvg, nActualTimespan);
	// ---end avg

	int nActualTimespanMax = nTargetTimespan*99/76; // 1.3026 ~30% diff limiter
        int nActualTimespanMin = nTargetTimespan*76/99;
...
*/
        ...
        // Genesis block
        const char* pszTimestamp = "15 December 2013, Drudge Report, China Lands on Moon!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce   = 863476890; 

        if (fTestNet)
        {
            block.nTime    = 1387098870;
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0x1b15154c7efe12419508ddbbeaafc7b001ffcfc487b2dfe4eebba120d8610a0d"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AtomMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AtomMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("AtomMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("atom-miner");
    ...
    try { loop {
        ...
        printf("Running AtomMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
